...
public class WebSshFileController {
    ...
    private void downloadSingleFile(String path, String sessionId,
        ...
        try {
            ...
            response.setContentType("application/octet-stream");
            response.setHeader("Content-Disposition",
                    "attachment;filename=\"" + urlEncodeUtf8(filename) + "\"");
    ...
    private void downloadDirectory(String path, String sessionId,
        ...
        try {
            ...
            response.setContentType("application/octet-stream");
            response.setHeader("Content-Disposition",
                    "attachment;filename=\"" + urlEncodeUtf8(downloadName) + "\"");
    ...
    /**
     * URL 编码（UTF-8）
     * <p>JDK 8 的 {@link URLEncoder#encode(String, String)} 接受字符串编码名，
     * 不接受 Charset 参数（Charset 重载是 Java 10+ 才有的）。这里用字符串形式并
     * 把 checked exception 包装为 RuntimeException（UTF-8 是 JVM 必备字符集，永远不会抛）。</p>
     */

...
let credentialFailCount = {};    // 凭据失败计数 {hostname: count}
const maxCredentialRetries = 5;  // 最大重试次数
...
function showCredentialDialog() {
    ...
    // 检查是否已达重试上限
    const failCount = credentialFailCount[currentHost] || 0;
    if (failCount >= maxCredentialRetries) {
        const at = getActiveTab();
        if (at && at.terminal) {
            at.terminal.write('\r\n\x1b[31mSSH 认证失败次数已达上限（5次），请刷新页面后重试\x1b[0m\r\n');
        }
        document.getElementById('fileList').innerHTML =
            '<div class="error-message">SSH 认证失败次数已达上限，请刷新页面后重试</div>';
        return;
    }
...
async function submitCredentials() {
    ...
    if (curHostInfo && !curHostInfo.needCredentials) {
        credentialFailCount[currentHost] = 0;
    ...
    try {
        ...
        if (result.success) {
            ...
            // 认证失败：显示错误、保持对话框、增加失败计数
            credentialFailCount[currentHost] = (credentialFailCount[currentHost] || 0) + 1;
            const remaining = maxCredentialRetries - credentialFailCount[currentHost];

            if (remaining <= 0) {
                document.getElementById('credentialError').textContent =
                    'SSH 认证失败次数已达上限（5次），请刷新页面后重试';
                document.getElementById('credentialError').style.display = 'block';
                submitBtn.disabled = true;
                submitBtn.textContent = '已达上限';
            } else {
                document.getElementById('credentialError').textContent =
                    'SSH 认证失败，用户名或密码错误（剩余重试次数：' + remaining + '）';
                document.getElementById('credentialError').style.display = 'block';
                submitBtn.disabled = false;
                submitBtn.textContent = '连接';
                // 清空密码，方便重新输入
